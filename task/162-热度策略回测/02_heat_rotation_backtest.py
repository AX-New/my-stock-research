...
def main():
    t0 = time.time()
    logger.info("=" * 60)
    logger.info("热度轮转策略回测 v2 - 单仓位全市场轮转")
    logger.info("=" * 60)
    ...
    # 5. 参数敏感性分析
    sens_df = sensitivity_analysis(heat_df, price_df, trading_days, index_df)
    ...
    if len(sens_df) > 0:
        report += "\n## 八、参数敏感性分析\n\n"
        report += "| 回看 | 买入 | 卖出 | 最大持仓 | 成交额门槛 | 总收益 | 年化 | 最大回撤 | 夏普 | 超额收益 | 笔数 | 胜率 | 每笔均收 |\n"
        report += "|------|------|------|---------|----------|--------|------|---------|------|---------|------|------|--------|\n"
        for _, r in sens_df.iterrows():
            report += (f"| {r['lookback']} | {r['buy_th']} | {r['sell_th']} | {r['max_hold']:.0f} | "
                       f"{r['min_deal']/1e8:.1f}亿 | {r['total_return']:.1f}% | {r['annual_return']:.1f}% | "
                       f"{r['max_drawdown']:.1f}% | {r['sharpe']:.2f} | {r['excess_total']:.1f}% | "
                       f"{r['trades']} | {r['win_rate']:.0f}% | {r['avg_return']:.2f}% |\n")
        report += "\n"

    # 7. 保存结果
    output_dir = os.path.dirname(os.path.abspath(__file__))

    report_path = os.path.join(output_dir, '热度轮转策略回测报告.md')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(report)
    logger.info(f"报告: {report_path}")

    trade_path = os.path.join(output_dir, 'rotation_trades.csv')
    trade_df.to_csv(trade_path, index=False, encoding='utf-8-sig')
    logger.info(f"交易明细: {trade_path}")

    equity_path = os.path.join(output_dir, 'rotation_equity.csv')
