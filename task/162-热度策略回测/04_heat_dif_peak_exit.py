"""
热度轮转策略回测 - DIF极值出场版本（单仓位全市场轮转）

基于 03_heat_rotation_no_timeout.py 修改：
- 入场逻辑不变：heat_position >= 0.8, rank_surge排序, 成交额>=5000万
- 出场逻辑改为DIF见顶信号（M4）：DIF连降2根视为DIF见顶，次日卖出
- 无超时强制平仓，仅在DIF见顶信号或期末平仓

【DIF见顶判断（M4）】
==========================================
MACD指标中的DIF线（快线-慢线），当DIF连续2天下降时，认为DIF已经见顶，
趋势可能转弱，此时卖出。
...
计算方法：
  ...
  DIF见顶条件：DIF_today < DIF_yesterday AND DIF_yesterday < DIF_day_before
  即DIF连续2天走低 → 顶部已形成 → 次日卖出

注意：持仓至少1天后才检查DIF信号，避免刚买入就因历史DIF下降而卖出。
"""
...
def simulate_rotation(heat_position, rank_pivot, deal_pivot, rank_surge,
    ...
    卖出条件（仅DIF见顶）：
    - 持仓至少1天后，检查DIF是否连续下降2天（M4信号）
    - DIF_today < DIF_yesterday AND DIF_yesterday < DIF_day_before_yesterday
    - 满足则次日卖出
    """
    ...
    for i, today in enumerate(td_list):
        ...
        if holding:
            ...
            # 持仓至少1天后才检查DIF信号（避免刚买入就因历史DIF下降而卖出）
            if hold_days >= 1:
                # 获取DIF连续3天的值：今天、昨天、前天
...
def build_report(trade_df, equity_df, metrics):
    ...
    lines.append("# 热度轮转策略回测报告（DIF极值出场）")
    ...
    lines.append("利用东财人气排名的均值回归特性选股，用MACD DIF指标判断出场时机。")
    lines.append("当一只股票的热度跌到近期最低点时买入，当DIF出现见顶信号（连续下降）时卖出，")
    lines.append("然后立即从全市场选出下一只\"最冷门\"的股票买入。")
    lines.append("**资金始终满仓运转，没有空窗期。**")
    lines.append("")
    lines.append("### 1.2 交易流程")
    lines.append("```")
    lines.append("每个交易日收盘后：")
    lines.append("  1. 如果持仓中 → 检查是否触发DIF见顶卖出条件")
    lines.append(f"     - DIF连降{DIF_DECLINE_BARS}根（M4信号）：")
    lines.append("       DIF_today < DIF_yesterday AND DIF_yesterday < DIF_day_before")
    lines.append("     - 持仓至少1天后才检查（避免刚买入就因历史DIF下降卖出）")
    ...
    if len(trade_df) > 0:
        ...
        lines.append("|---------|------|------|---------|------------|")
        for reason, label in [('dif_peak_m4', 'DIF见顶(M4)'),
                              ('end_of_period', '期末平仓')]:
    ...
    if len(trade_df) > 0:
        ...
        for idx, row in trade_df.iterrows():
            ...
            reason_map = {'dif_peak_m4': 'DIF见顶(M4)', 'end_of_period': '期末'}
    ...
    lines.append("|------|--------|---------|---------|------|------|---------|")
    lines.append(f"| **DIF见顶出场（本策略）** | **{metrics['total_return']:.2f}%** | "
...
def main():
    ...
    report_path = os.path.join(output_dir, '热度轮转策略回测报告_DIF出场.md')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(report)
    logger.info(f"报告: {report_path}")

    trade_path = os.path.join(output_dir, 'rotation_trades_dif_exit.csv')
    trade_df.to_csv(trade_path, index=False, encoding='utf-8-sig')
    logger.info(f"交易明细: {trade_path}")

    equity_path = os.path.join(output_dir, 'rotation_equity_dif_exit.csv')
