"""DIF极值信号有效性深度分析（ATR标准化版本）

核心改进:
  使用 DIF/ATR(20) 替代 |DIF| 进行幅度分档，消除价格量纲差异，
  使不同价位的股票/指数在同一标准下可比。

核心问题:
1. DIF需要多大才算有意义的"极值"？（ATR标准化幅度 vs 信号质量）
...
用法:
  python analyze_dif_extreme_atr.py --level index
  python analyze_dif_extreme_atr.py --level industry
  python analyze_dif_extreme_atr.py --level stock
  python analyze_dif_extreme_atr.py --level index --codes 000001.SH:上证指数  # 单指数测试
"""
import argparse
import os
import sys
...
def calc_atr(high: np.ndarray, low: np.ndarray, close: np.ndarray, period: int = 20) -> np.ndarray:
...
def detect_extremes(df: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    新增: 计算ATR并输出 dif_atr = |DIF| / ATR(20) 用于标准化幅度分档
    """
    ...
    # 计算ATR（基于全量数据）
    atr_values = calc_atr(df["high"].values, df["low"].values, df["close"].values)
    ...
    for idx in peak_indices:
        ...
        records.append({
            ...
            "dif_atr": abs(float(row["dif"])) / float(atr_values[idx]) if atr_values[idx] > 0 else np.nan,
            "type": "peak",  # DIF极大值
...
def assign_magnitude_band(extremes: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    for ext_type in ["peak", "trough"]:
        ...
        subset = extremes.loc[mask, "dif_atr"].dropna()
        if len(subset) == 0:
            continue
        ...
        extremes.loc[bands.index, "mag_band"] = bands

    # 对于dif_atr为NaN但未被处理的行，标记为unknown
    nan_mask = extremes["dif_atr"].isna() & (extremes["mag_band"] == "")
    extremes.loc[nan_mask, "mag_band"] = "unknown"
...
def print_magnitude_quantiles(extremes: pd.DataFrame, freq: str):
    ...
    for ext_type in ["peak", "trough"]:
        ...
        # ATR标准化度量
        dif_atr_vals = subset["dif_atr"].dropna()
        if len(dif_atr_vals) > 0:
            qs = dif_atr_vals.quantile([0.25, 0.5, 0.75])
            print(f"  {type_cn} DIF/ATR 分位: Q25={qs[0.25]:.2f}, Q50={qs[0.5]:.2f}, "
                  f"Q75={qs[0.75]:.2f}, min={dif_atr_vals.min():.2f}, max={dif_atr_vals.max():.2f}, N={len(dif_atr_vals)}")
        # 同时输出原始abs_dif作为参考
        abs_vals = subset["abs_dif"]
        qs2 = abs_vals.quantile([0.25, 0.5, 0.75])
        print(f"  {type_cn} |DIF|(参考): Q25={qs2[0.25]:.2f}, Q50={qs2[0.5]:.2f}, "
              f"Q75={qs2[0.75]:.2f}, min={abs_vals.min():.2f}, max={abs_vals.max():.2f}")
...
def run_index_level(codes: list[tuple[str, str]] = None):
    ...
    print_section("Level 1: 大盘指数 DIF极值有效性分析（ATR标准化）")
...
def run_industry_level():
    """行业指数层级分析（按5大类别聚合）"""
    print_section("Level 2: 行业指数 DIF极值有效性分析（ATR标准化）")
...
def run_stock_level():
    """个股层级分析（全市场汇总统计 + 离散度）"""
    print_section("Level 3: 个股 DIF极值有效性分析（全市场前复权，ATR标准化）")
...
def main():
    parser = argparse.ArgumentParser(description="DIF极值信号有效性深度分析（ATR标准化）")
