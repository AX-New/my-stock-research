# my_trend 数据表目录
**最后更新**: 20260320 23:30（基于腾讯云 MySQL 实际抓取）
...
| `em_hot_keyword` | ~4600 | 每日19:00 | 个股热门关键词（概念名称/概念代码/热度值） |
...
| `heat_change_top` | ~20 | 盘中+10min | 热度飙升Top20（rank变化/入选时间/价格/涨跌幅/量比/换手率） |

## 盘中Top100监控（hot 模块）

| 表名 | 行数 | 更新频率 | 说明 |
|------|------|---------|------|
| `intraday_heat_snapshot` | 0 | 盘中×8次 | AkShare Top100快照（排名/前次排名/变化/价格/量比/成交额） |
| `sector_news` | 0 | 盘中×8次 | 涨幅前20板块实时新闻（板块类型/标题/内容/来源/时间） |
...
```
盘中（交易日，非交易日自动跳过）:
  09:40~14:40 每小时  heat_live.main         → popularity_rank_live
  09:50~14:50 每小时  heat_live.analyze       → heat_change_top
  09:45~14:45 每小时  hot.main --all          → intraday_heat_snapshot + sector_news

收盘后:
  17:00         heat.main                → popularity_rank
  17:20         heat.minute              → heat_stock_minute
  19:00         heat.main --keyword      → em_hot_keyword
